...
* 你正在协助的对象是 **Cyo**。
* 假设 Cyo 是一名经验丰富的资深后端 / 数据库工程师，熟悉 Rust、Go、Python 等主流语言及其生态。
* Cyo 重视“Slow is Fast”，关注点在于：推理质量、抽象与架构、长期可维护性，而不是短期速度。
* 你的核心目标：
...
- `src/main.rs`: CLI parsing and the main render/animation loop.
- `src/animation/`: animation styles (one file per style) and shared style utilities in `styles.rs`.
- `src/system/`: system information collection plus platform ASCII logos.
- `src/util/`: shared helpers (notably ANSI parsing in `ansi.rs`).
- `tests/`: integration tests (currently a smoke test that runs the binary).
- Root `*.png`/`*.mp4`: README showcase assets (keep additions intentional and reasonably sized).

## Build, Test, and Development Commands

- `cargo run -- --style neon`: run locally (pass args after `--`).
- `cargo run -- --fetch`: one-shot output (used by the smoke test).
- `cargo build`: debug build.
- `cargo build --release`: optimized build in `target/release/`.
- `cargo test`: run unit + integration tests.
- `cargo fmt` / `cargo fmt -- --check`: format or verify formatting.
- `cargo clippy --all-targets --all-features -- -D warnings`: match CI linting (warnings are errors).

## Coding Style & Naming Conventions

- Rust 2024 edition (`edition = "2024"`); keep code `rustfmt`-clean and clippy-friendly.
- Use Rust standard naming: `snake_case` (modules/functions), `CamelCase` (types/traits), `SCREAMING_SNAKE_CASE` (consts).
- Prefer small, testable helpers; keep terminal I/O and platform probing localized (usually under `src/system/`).
- Avoid unnecessary dependencies; when adding/changing deps, commit the updated `Cargo.lock`.

## Testing Guidelines

- Use the built-in Rust test harness (`cargo test`).
- Add integration tests under `tests/` when behavior is observable via the CLI; prefer fast flags like `--fetch`, `--frame`, `--duration`, and deterministic `--seed`.
- Follow the existing pattern in `tests/smoke.rs` using `env!("CARGO_BIN_EXE_neonfetch")` to execute the compiled binary.

## Commit & Pull Request Guidelines

- Recent history trends toward Conventional Commit-style subjects: `feat(animation): ...`, `fix(system): ...`, `chore: ...` (keep the first line short and specific).
- PRs should include: a clear description, how to manually verify (example commands), and updates to `README.md`/assets when user-facing output changes (new styles, new flags, screenshots).
- Before requesting review, ensure `cargo fmt -- --check`, `cargo clippy ... -D warnings`, and `cargo test` pass.

## Release Notes (Optional)

- The GitHub Actions release workflow triggers on tags like `v0.1.0`; keep `Cargo.toml` version and the tag in sync.
