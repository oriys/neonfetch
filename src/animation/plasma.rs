...
pub fn calculate_plasma_color_with_palette(
    time: f32,
    row: usize,
    col: usize,
    term_w: usize,
    ...
    if palette.is_default() {
        return calculate_plasma_color_at(time, row, col, term_w, term_h, speed);
    }
    if term_w == 0 || term_h == 0 {
        return (0, 0, 0);
    }
    ...
    let vn = (v + 3.0) / 6.0;
    let hue = (vn * 300.0 + time * 25.0) % 360.0;
    let sat = 0.55 + (vn - 0.5).abs() * 0.9;
    let mut val = 0.30 + (vn.powf(0.8)) * 0.70;
    val *= (time * 0.6).sin() * 0.05 + 0.97;
    palette.sample_tinted(hue / 360.0, sat.min(1.0), val.min(1.0))
}
