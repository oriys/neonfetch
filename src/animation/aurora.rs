...
pub fn calculate_aurora_color_with_palette(
    time: f32,
    row: usize,
    col: usize,
    term_w: usize,
    ...
    if palette.is_default() {
        return calculate_aurora_color_at(time, row, col, term_w, term_h, speed);
    }
    if term_w == 0 || term_h == 0 {
        return (0, 0, 0);
    }
    ...
    let c1 = ((t * 0.17).sin() * 0.5 + 0.5) * 1.1 - 0.05;
    let c2 = (t * 0.11 + 1.7).sin() * 0.5 + 0.5;
    let c3 = (t * 0.07 + 3.1).sin() * 0.5 + 0.5;
    let band_width = 0.22;
    let falloff = |center: f32, pos: f32| -> f32 {
    ...
    let und2 = ((x * 5.5) - t * 2.2).sin() * 0.5 + 0.5;
    let und3 = ((x * 6.5) + t * 1.7).sin() * 0.5 + 0.5;
    let sky_grad = ((1.0 - y) * 0.9 + 0.1).powf(1.4);
    let mix1 = b1 * und1;
    let mix2 = b2 * und2;
    let mix3 = b3 * und3;
    let intensity = (mix1 + mix2 * 0.9 + mix3 * 1.1) * 0.9 * sky_grad;
    ...
    let val = (i.powf(0.9) * breath).clamp(0.05, 1.0);
    palette.sample_tinted(hue / 360.0, sat, val)
}
