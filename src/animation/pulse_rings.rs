...
pub fn calculate_pulse_rings_color_with_palette(
    time: f32,
    row: usize,
    col: usize,
    term_w: usize,
    ...
    if palette.is_default() {
        return calculate_pulse_rings_color_at(time, row, col, term_w, term_h, speed);
    }
    if term_w == 0 || term_h == 0 {
        return (0, 0, 0);
    }
    ...
    let y = row as f32 + 0.4;
    let dx = x - cx;
    let dy = y - cy;
    let dist = (dx * dx + dy * dy).sqrt();
    let ring_spacing = 6.5_f32;
    let ring_speed = 24.0;
    let base = dist - t * ring_speed;
    let phase = base / ring_spacing;
    let frac = phase - phase.floor();
    let d = frac.min(1.0);
    let centered = (d - 0.5).abs() * 2.0;
    let thickness = 0.55;
    let ring_env = (-(centered * centered) / (thickness * thickness)).exp();
    let radial_fade = (1.0 - (dist / ring_spacing) * 0.06).clamp(0.20, 1.0);
    let intensity = ring_env * radial_fade;
